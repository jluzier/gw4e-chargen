...
- **Share a generated character.** Let a player send/publish their finished character to someone else (e.g. a shareable link or exported file). Likely blocked on some form of the persistence feature above landing first, so a character has a stable identity/storage location to share *to*.
- **Reroll base attributes (1x).** Allow a single one-time reroll of the full base-attribute set (PS/DX/CN/MS/IN/CH/SN) during the Attributes step, for players unhappy with their initial roll. Needs a decision on scope: reroll all seven together vs. per-attribute, and whether it's offered before or after mutations are applied.
- **Name generator prompt/link-out.** Surface a prompt or external link (or a lightweight built-in generator) to help a player pick a character name, rather than leaving the name field blank with no assistance.
- **Character portrait via image generator.** Prompt or link-out to an image-generation tool, pre-seeded with a description built from the character's genotype, attributes, mutations, class, etc., so a player can get AI art of their character. Out of scope for now: hosting/storing the resulting image (ties into persistence above).
